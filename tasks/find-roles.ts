...
const KNOWN_ROLES: { [key: string]: string } = {
  ...
  "0xd83740310a408b03a53117fdd07e226c91fa1daa4d57713b36cc45eccac04b43": "UPGRADER_ROLE"
};

function tryIdentifyRoleName(roleId: string, contract: Contract): string | undefined {
...
task("find-roles", "Find all roles and their holders for an AccessControl contract")
  ...
  .setAction(async (taskArgs: FindRolesTaskArgs, hre: HardhatRuntimeEnvironment) => {
    const { address: contractAddress, abi: abiPath, fromBlock: fromBlockArg, toBlock: toBlockArg } = taskArgs;
    ...
    const { fromBlock, toBlock } = await getBlockParams(hre, contractAddress, fromBlockArg, toBlockArg);
    ...
    if (!hasRoleGrantedEvent || !hasRoleRevokedEvent) {
      throw new Error("Contract does not implement AccessControl interface. Missing RoleGranted/RoleRevoked events.");
    }

    // Retrieve both RoleGranted and RoleRevoked events
    console.log("Fetching RoleGranted events...");
    const grantedLogs = await getEventLogs(contract, "RoleGranted", fromBlock, toBlock);
    
    console.log("Fetching RoleRevoked events...");
    const revokedLogs = await getEventLogs(contract, "RoleRevoked", fromBlock, toBlock);
    ...
    console.log(`Found ${grantedEvents.length} RoleGranted events and ${revokedEvents.length} RoleRevoked events.`);
    ...
    for (const event of grantedEvents) {
      ...
      const roleIdentifier = event.eventParameters.role || event.eventParameters.roleId;
      const account = event.eventParameters.account;
      
      if (!roleIdentifier || !account) {
        console.warn(`Missing role identifier or account in event: ${JSON.stringify(event)}`);
        continue;
      }
    ...
    for (const event of revokedEvents) {
      ...
      if (roleHolders[roleIdentifier] && roleHolders[roleIdentifier][account]) {
    ...
    console.log(`Analysis complete. Found ${roleCount} roles. Results written to ${outputPath}`);
  });
